...
from vidbot.models.clip.interpolate import interpolate_positional_embedding
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        heads: int,
        output_dim: int,
        output_indices: list = [6, 7, 8, 9, 10],
    ):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
    ...
    def forward(self, x: torch.Tensor, patch_output: bool = False, return_dict=False):
        output_dict = {}
        ...
        if patch_output:
            *layers, last_resblock = self.transformer.resblocks
            ...
            for ii, layer in enumerate(layers):
                _x = layer(x)  # Results after layer normalization
                # TODO: add f"res{ii}" output dict here!!!
                if return_dict and ii in self.output_indices:
                    res_i = ii - 5
                    output_dict[f"res{res_i}"] = _x.permute(1, 0, 2)
